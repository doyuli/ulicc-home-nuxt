...
const volumeNum = ref(musicStore.value.volume ? musicStore.value.volume : 0.7);
...
// 音乐上下曲
const changeMusicIndex = (type: any) => {
...
watch(
    () => volumeNum.value,
    (value) => {
        musicStore.value.volume = value;
        playerRef.value.changeVolume(musicStore.value.volume);
    }
);
</script>
...
<template>
    <!-- 音乐控制面板 -->
    <div class="music" @mouseenter="volumeShow = true" @mouseleave="volumeShow = false" v-show="musicStore.openState">
        <div class="btns">
            <span @click="musicListShow = true">音乐列表</span>
            <span @click="musicStore.openState = false">回到一言</span>
        </div>
        <div class="control">
            <go-start theme="filled" size="30" fill="#efefef" @click="changeMusicIndex(0)" />
            <div class="state" @click="changePlayState">
                <play-one theme="filled" size="50" fill="#efefef" v-show="!playerStore.state" />
                <pause theme="filled" size="50" fill="#efefef" v-show="playerStore.state" />
            </div>
        ...
        <div class="menu">
            <div class="name" v-show="!volumeShow">
                <span>{{
                    playerStore.title
                    ? playerStore.title + " - " + playerStore.artist
                    : "未播放音乐"
                }}</span>
            </div>
